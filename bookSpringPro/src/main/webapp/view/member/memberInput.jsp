...
<link rel="stylesheet" href="<%=request.getContextPath() %>/css/memberCss/memberInput.css">
</head>
<script>
//회원가입관련 데이터를 채우지 않았을 경우 실행
function checkValue(){
...
//아이디 중복체크 창을 띄운다.
function openidChk() {
	window.open("<%=request.getContextPath() %>/member/idChkForm", 
			'idChk', 'top=10, left=10, width=500, height=300')
}
...
<form method="post" action="<%=request.getContextPath() %>/member/memberInputPro" name="userInfo"
...
<table class="table">
	<tr>
		...
		<input type="button" value="중복체크" class="button" onclick="openidChk()">
		</td>
	</tr>
